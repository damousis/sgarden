...
const Card = ({
	...
	const exportSelector = chartName ? `export-csv-${chartName}` : "";

	const handleCsvExport = () => {
		exportRowsToCsv({
			rows: csvData,
			filename: csvFileName || chartName || "chart-data",
		});
	};
	...
	return (
		...
		>
			{showCsvExport && chartName && (
				...
				>
					<Button
						...
						aria-label={`Export CSV for ${chartName}`}
					>
						Export CSV
					</Button>
				</Box>
			...
			{titleExists && (
				...
				>
					{typeof title === "string" ? (
						<Typography variant="body1" component="h2" fontSize={titleFontSize}>
							{title}
						</Typography>
					) : (
						title
			...
			{footerExists && (
				...
				>
					{typeof footer === "string" ? (
						<Typography variant="h6" component="h2" fontSize={footerFontSize}>
							{footer}
						</Typography>
					) : (
						footer
